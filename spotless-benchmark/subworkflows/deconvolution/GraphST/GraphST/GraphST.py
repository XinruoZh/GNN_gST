...
import random
import numpy as np
from .model import Encoder, Encoder_sparse, Encoder_map, Encoder_sc, Encoder_GAT, Encoder_SGC, Encoder_GATv2
...
class GraphST():
    ...
    def train(self):
        if self.datatype == 'GAT':
           print("Initializing Encoder_GAT...")
           self.model = Encoder_GAT(self.dim_input, self.dim_output, self.graph_neigh).to(self.device)
        elif self.datatype == 'GATv2':
            print("Initializing Encoder_GATv2...")
            self.model = Encoder_GATv2(self.dim_input, self.dim_output, self.graph_neigh).to(self.device)
        elif self.datatype == 'SGC':
           print ("Initializing Encoder_SGC...")
           self.model = Encoder_SGC(self.dim_input, self.dim_output, self.graph_neigh).to(self.device)
